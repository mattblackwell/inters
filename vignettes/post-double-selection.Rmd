...
single <- felm(Protest ~ remit*dict + l1gdp + l1pop + l1nbr5 + l12gr + l1migr
               ...
               data = remit)
lmtest::coeftest(single, vcov = single$clustervcv)[c("remit", "remit:dict"),]
```

Next, we compare this single-interaction model to a model that fully interacts the moderator with the entire set of controls, including the fixed effects. @BlaOls19 call this the *fully moderated model*.

```{r fully}
fully <- felm(Protest ~ dict * (remit + l1gdp + l1pop + l1nbr5 + l12gr + l1migr +
                                  elec3 + factor(period) + factor(cowcode))
              | 0 | 0 | caseid, data = remit)
lmtest::coeftest(fully, vcov = fully$clustervcv)[c("remit", "dict:remit"),]
```
